...
#
#Alterado por Bruno Souza (bruno.stish@gmail.com)
#Adaptado para funcionar com strongSwan no pfSense 2.3.1
#
...
ping_tunnel() {
        ...
        alive=`$FPINGBIN -r 1 -S $IP_SRC $IP_DEST | grep alive | wc -l`
...
ping_tunnel_rtt() {
        ...
        rtt=`$FPINGBIN -r 1 -e -S $IP_SRC $IP_DEST`
...
test_tunnel() {
        ...
        then
           ...
           tunneltest=`$IPSECCMD status $CONN | grep -e "INSTALLED" | wc -l`
        else
